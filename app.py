...
# மிக உறுதியான மொழிபெயர்ப்பு (பிழையின்றித் தமிழில் மாற்ற)
def safe_translate(text):
    if not text or len(text) < 5: return "தகவல் இல்லை."
    try:
        # தமிழுக்கு மொழிபெயர்க்கும் செட்டிங்ஸ்
        translator = Translator(to_lang="ta", from_lang="en")
        # 500 எழுத்துக்கள் வரை மட்டும் மொழிபெயர்க்கும் (நிலைத்தன்மைக்காக)
...
# 3. பிரீமியம் CSS (Dark Theme & Compact UI)
st.markdown("""
    <style>
...
try:
    ...
    if 'longName' in info:
        ...
        with tab3:
            ...
            fig_pie = go.Figure(data=[go.Pie(labels=['Promoters', 'Institutions', 'Others'], values=[p_val, inst_val, 100-(p_val+inst_val)], hole=.5, marker=dict(colors=['#ffd700', '#58a6ff', '#2ea043']))])
        ...
        with tab4:
            ...
            if roe > 0.15: st.success("Strong Fundamental Strength 🚀 (நல்ல எதிர்காலம்)")
