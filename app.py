...
st.markdown("""
    ...
    .rating-card { padding: 20px; border-radius: 15px; text-align: center; margin: 10px 0; border: 2px solid rgba(255,255,255,0.1); }
    .score-text { font-size: 45px; font-weight: 900; margin: 5px 0; }
    </style>
...
# 5. டேப்கள் (அனைத்து வசதிகளும் சேர்க்கப்பட்டுள்ளது)
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    "📊 பகுப்பாய்வு", "🤝 Shareholding", "⭐ ரேட்டிங்", "📅 Actions", "🔮 கணிப்பு", "🗞️ செய்திகள்", "📌 Watchlist"
])
...
if stock_loaded:
    ...
    with tab1:
        ...
        with c1:
            ...
            st.markdown(f'<div class="metric-row"><div><span class="m-label">P/B RATIO</span><br><span class="m-value">{info.get("priceToBook", "N/A")}</span></div></div>', unsafe_allow_html=True)
        with c2:
            ...
            st.markdown(f'<div class="metric-row"><div><span class="m-label">PEG RATIO</span><br><span class="m-value">{info.get("pegRatio", "N/A")}</span></div></div>', unsafe_allow_html=True)
    ...
    with tab2:
        st.markdown("### 🤝 Shareholding Pattern (பங்குதாரர் விபரம்)")
    ...
    with tab4:
        ...
        st.table(stock_obj.actions.tail(10).sort_index(ascending=False))

    with tab5:
        st.markdown("### 🔮 Forecast")
        target = info.get('targetMeanPrice')
        if target:
            st.metric("Analyst Target", f"₹{target:,.2f}", f"{((target-ltp)/ltp)*100:.2f}% Upside")
        else: st.write("கணிப்புகள் இல்லை.")

    with tab6:
        st.markdown("### 🗞️ செய்திகள்")
        for n in stock_obj.news[:5]:
            st.markdown(f'<div style="padding:10px; border-left:4px solid #39FF14; margin-bottom:10px; background:#161b22;"><a href="{n.get("link","#")}" target="_blank" style="color:#39FF14; font-weight:bold; text-decoration:none;">{n.get("title")}</a></div>', unsafe_allow_html=True)
